    ...
    public sealed class LocaleIT : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.UseOverlapWarningColor)), "<Abilitato Ã¨ consigliato>\nMantiene visibile il contorno vanilla rosso salmone del gioco quando il posizionamento di oggetti o reti Ã¨ bloccato da elementi sovrapposti.\nI limiti di area, come le guide del raggio delle fattorie dellâ€™Industria specializzata, non vengono modificati.\n\nFunziona con tutti i modi Bulldozer + strade e non sovrascrive il colore personalizzato salvato." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.UseCustomColorsForNetLanes)), "<Abilitato Ã¨ consigliato>\nUsa il colore/trasparenza HC salvato mentre posizioni elementi di dettaglio NetLane come recinzioni, siepi, segnaletica e strumenti simili basati sulle corsie.\n\n- Le strade normali seguono comunque lâ€™impostazione Bulldozer + strade scelta dal menu a discesa.\n- Disabilita questa opzione se vuoi che quegli strumenti usino invece il blu vanilla del gioco.\n- Il colore di errore da sovrapposizione ha ancora la prioritÃ  quando Ã¨ abilitato (colore errore vanilla = rosso salmone)." },

                // Darker panel
                { m_Settings.GetOptionLabelLocaleID(nameof(HoverColorsSettings.UseDarkerPanel)), "Pannello piÃ¹ scuro" },
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.UseDarkerPanel)), "Abilitato usa <Pannello scuro>: creato per i giocatori con UI legacy; puÃ² essere usato anche nella UI moderna se preferisci un pannello piÃ¹ scuro.\nDisabilitato usa <Pannello standard>: stile Hover Colors traslucido personalizzato.\n- Aspetto piÃ¹ chiaro e moderno.\n- Migliore per la maggior parte dei giocatori che usano la nuova UI moderna del gioco.\n\nProvali entrambi e scegli quello che preferisci! Cambia solo lo sfondo di questo pannello del mod, non la UI del gioco." },
                // Guidelines opacity slider
                { m_Settings.GetOptionLabelLocaleID(nameof(HoverColorsSettings.GuidelineOpacityPercent)), "OpacitÃ  guide (alpha)" },
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.GuidelineOpacityPercent)), "Controlla lâ€™opacitÃ  delle guide di allineamento tratteggiate, utile mentre posizioni strade, recinzioni, props, ecc.\n\n**100%** mantiene lâ€™aspetto vanilla predefinito.\n**PiÃ¹ basso** rende le guide piÃ¹ trasparenti.\n**0%** le nasconde completamente - <Non consigliato>.\nSi consiglia di restare sopra il 15%, altrimenti diventa difficile vedere cosa succede.\nLo stesso cursore Ã¨ nel pannello del mod in cittÃ . Sono sincronizzati;\nse cambi questo, cambia anche quello in cittÃ ." },

                // Keybinds
                { m_Settings.GetOptionLabelLocaleID(nameof(HoverColorsSettings.TogglePanelBinding)), "Apri/chiudi pannello principale" },
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.TogglePanelBinding)), "Scorciatoia da tastiera per aprire / chiudere il pannello colore degli oggetti hover in cittÃ ." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.OpenParadox)), "Apri la pagina Paradox Mods dellâ€™autore." },

                { m_Settings.GetOptionLabelLocaleID(nameof(HoverColorsSettings.MochiDedicationText)), "In amorevole memoria di Mochi." },
                { m_Settings.GetOptionDescLocaleID(nameof(HoverColorsSettings.MochiDedicationText)), "Questo mod Ã¨ dedicato a Mochi. Era una cagnolina amatissima, adottata allâ€™etÃ  di 7 anni,\ne ha donato 13 anni di amore e gioia. Questo mod non sarebbe stato possibile senza Mochi." },
            };
        }
