...
// Purpose: Small color/style helpers for MochiColorPickerPanel.
...
// Preset previews ignore alpha for legibility; saved preset still applies alpha in-game.
export const presetPreviewStyle = (c: Color): React.CSSProperties => ({
...
export const holdBarStyle = (progress: number): React.CSSProperties => ({
...
export const makeCompactSwatchStyle = (useDarkerPanel: boolean) => {
    const compactSwatchStyle = (c: Color, hovered = false): React.CSSProperties => {
    ...
    return {
        compactSwatchStyle,
        guidelineShellStyle: compactSwatchStyle,
        ownerShellStyle: compactSwatchStyle,
    };
};
